...
#ifdef HAVE_M4IPH_HWADD_SDR_MEM
...
gst_shvideodec_sink_event (GstPad * pad, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) 
  {
    case GST_EVENT_EOS:
    {
      ...
      // We don't have buffering so just inform application about EOS
...
gst_shvideodec_chain (GstPad * pad, GstBuffer * inbuffer)
{
  ...
  dec->current_timestamp = GST_BUFFER_TIMESTAMP (inbuffer);
  GST_LOG_OBJECT(dec,"timestamp: %llu duration: %llu",
    GST_TIME_AS_MSECONDS(dec->current_timestamp),
    GST_TIME_AS_MSECONDS(GST_BUFFER_DURATION (inbuffer)));

  ret = gst_shvideodec_decode (dec, inbuffer);
...
static GstFlowReturn
gst_shvideodec_decode (Gstshvideodec * dec, 
		       GstBuffer     * inbuffer)
{
  int used_bytes;
  ...
  if (inbuffer==NULL) 
  {
    GST_DEBUG_OBJECT(dec,"%s called with null inbuffer",__FUNCTION__);
  }
  else
  {
    GST_LOG_OBJECT(dec,"%s called\n",__FUNCTION__);
  ...
  dec->y_addr = read_reg(&vpu_mmio,VPDYR);
  dec->c_addr = read_reg(&vpu_mmio,VPDCR);
#endif
  used_bytes = shcodecs_decode(dec->decoder,
		  GST_BUFFER_DATA (inbuffer),
		  GST_BUFFER_SIZE (inbuffer));

  GST_LOG_OBJECT(dec,"Input buffer size: %d used: %d decoded %d frames",
   GST_BUFFER_SIZE (inbuffer),used_bytes,
			       ...
			       void * user_data)
{
...
#ifndef HAVE_M4IPH_HWADD_SDR_MEM
  sh_vidix.offset.y=dec->y_addr;
  sh_vidix.offset.u=dec->c_addr;
#else
  sh_vidix.offset.y=(unsigned)m4iph_hwadd_sdr_mem(y_buf);
  sh_vidix.offset.u=(unsigned)m4iph_hwadd_sdr_mem(c_buf);
#endif

  if (dec->waiting_for_first_frame)
  { 
    dec->start_time = gst_clock_get_time(dec->clock);
    dec->first_timestamp = dec->current_timestamp;
  ...
  stamp_diff = 
    GST_TIME_AS_MSECONDS(dec->current_timestamp) - GST_TIME_AS_MSECONDS(dec->first_timestamp);
  ...
  GST_LOG_OBJECT(dec,"Frame number: %d time from start: %llu stamp diff: %llu",frame_number,time_diff,stamp_diff);
